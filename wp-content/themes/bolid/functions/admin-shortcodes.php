...
if ( !function_exists( 'tmnf_shortcode_stylesheet' ) AND get_option( 'vergo_disable_shortcodes') != "true" ) {
	function tmnf_shortcode_stylesheet() {
		echo "<!-- Vergo Shortcodes CSS -->\n";
...
add_filter( 'the_content', 'tmnf_sc_formatting', 7 );
...
function tmnf_shortcode_toggle ( $atts, $content = null ) {
		...
		$defaults = array(
							'title_open' => __( 'Hide the Content','vergo'),
							'title_closed' => __( 'Show the Content','vergo'),
							'hide' => 'yes',
							...
							'read_more_text' => __( 'Read More','vergo'),
							'read_less_text' => __( 'Read Less','vergo')
						);
...
function tmnf_shortcode_contactform ( $atts, $content = null ) {
		...
		$defaults = array(
						...
						'subject' => __( 'Message via the contact form','vergo'),
						'button_text' => apply_filters( 'tmnf_contact_form_button_text', __( 'Submit','vergo') )
						);
		...
		if ( ( count( $_POST ) > 3 ) && isset( $_POST['submitted'] ) ) {
			...
			$error_responses = array(
									'contactName' => __( 'Please enter your name','vergo'),
									'contactEmail' => __( 'Please enter your email address (and please make sure it\'s valid)','vergo'),
									'contactMessage' => __( 'Please enter your message','vergo')
									);
			...
			if ( count( $error_messages ) ) {} else {
				...
				// Send the e-mail.
				$headers = __( 'From: ','vergo') . $default_fields['contactName'] . ' <' . $default_fields['contactEmail'] . '>' . "\r\n" . __( 'Reply-To: ','vergo') . $default_fields['contactEmail'];
		...
		if( isset( $emailSent ) && $emailSent == true ) {
			...
			$html .= do_shortcode( '[box type="tick"]' . __( 'Your email was successfully sent.','vergo') . '[/box]' );
		...
		if( count( $error_messages ) ) {
			...
			$html .= do_shortcode( '[box type="alert"]' . __( 'There were one or more errors while submitting the form.','vergo') . '[/box]' );
        ...
        if( $email == '' ) {
			...
			$html .= do_shortcode( '[box type="alert"]' . __( 'E-mail has not been setup properly. Please add your contact e-mail!','vergo') . '[/box]' );
		...
		if ( $email == '' ) {} else {
			...
			$html .= '<p><label for="contactName">' . __( 'Name','vergo') . '</label>' . "\n";
			...
			$html .= '<p><label for="contactEmail">' . __( 'Email','vergo') . '</label>' . "\n";
			...
			$html .= '<p class="textarea"><label for="contactMessage">' . __( 'Message','vergo') . '</label>' . "\n";
			...
			$html .= '<p class="inline"><input type="checkbox" name="sendCopy" id="sendCopy" value="true"' . $sendCopy . ' /><label for="sendCopy">' . __( 'Send a copy of this email to yourself','vergo') . '</label></p>' . "\n";
			...
			$html .= '<p class="screenReader"><label for="checking" class="screenReader">' . __('If you want to submit this form, do not enter anything in this field','vergo') . '</label><input type="text" name="checking" id="checking" class="screenReader" value="' . esc_attr( $checking ) . '" /></p>' . "\n";
...
{        
    ob_start();  
    get_template_part('/includes/lsc-services');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
{        
    ob_start();  
    get_template_part('/includes/lsc-clients');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
{        
    ob_start();  
    get_template_part('/includes/lsc-pricing_tabs3');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
{        
    ob_start();  
    get_template_part('/includes/lsc-pricing_tabs4');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
{        
    ob_start();  
    get_template_part('/includes/lsc-home-social');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
/*-----------------------------------------------------------------------------------*/
/* 18D. Latest Blog Section - [blog_latest]
/*-----------------------------------------------------------------------------------*/

add_shortcode('blog_latest', 'tmnf_shortcode_blog_latest');   
function tmnf_shortcode_blog_latest($attr, $content)
{        
    ob_start();  
    get_template_part('/includes/lsc-blog_latest');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
/*-----------------------------------------------------------------------------------*/
/* 18D. Featured Blog Section - [blog_featured]
/*-----------------------------------------------------------------------------------*/

add_shortcode('blog_featured', 'tmnf_shortcode_blog_featured');   
function tmnf_shortcode_blog_featured($attr, $content)
{        
    ob_start();  
    get_template_part('/includes/lsc-blog_featured');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
/*-----------------------------------------------------------------------------------*/
/* 18E. Portfolio Classic - [portfolio]
/*-----------------------------------------------------------------------------------*/

add_shortcode('portfolio', 'tmnf_shortcode_portfolio');   
function tmnf_shortcode_portfolio($attr, $content)
{        
    ob_start();  
    get_template_part('/includes/lsc-folio_latest');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
/*-----------------------------------------------------------------------------------*/
/* 18E. Portfolio Mosaic - [portfolio_featured]
/*-----------------------------------------------------------------------------------*/

add_shortcode('portfolio_featured', 'tmnf_shortcode_portfolio_featured');   
function tmnf_shortcode_portfolio_featured($attr, $content)
{        
    ob_start();  
    get_template_part('/includes/lsc-folio_featured');  
    $ret = ob_get_contents();  
    ob_end_clean();  
    return $ret;    
}
...
function tmnf_shortcode_socialicon ( $atts, $content = null ) {
	...
	if ( $profile_type != '' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $_profile_to_display ); $_alt_text = sprintf( __( 'My %s Profile','vergo'), $_alt_text ); } // End IF Statement
	...
	if ( $url == 'feed' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = __( 'Subscribe to our RSS feed','vergo'); } // End IF Statement
		...
		foreach ( $_supported_profiles as $k => $v ) {
			...
			if ( $host == $v ) {
				...
				if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $matches[1] ); $_alt_text = sprintf( __( 'My %s Profile','vergo'), $_alt_text ); } // End IF Statement
...
function tmnf_shortcode_twitter_follow ( $atts, $content = null ) {
	...
	$output = '<div class="shortcode-twitter-follow' . $allowed_floats[$float] . '"><a href="http://twitter.com/' . $username . '/"' . $tag_atts . ' class="twitter-follow-button">' . __( 'Follow','vergo') . ' ' . $username . '</a></div><!--/.shortcode-twitter-follow-->' . "\n";
