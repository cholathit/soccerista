...
/*-----------------------------------------------------------------------------------*/
/* Vergo Admin Interface - vergothemes_add_admin */
/*-----------------------------------------------------------------------------------*/
...
function vergothemes_add_admin() {

    global $query_string;
    global $current_user;
    $current_user_id = $current_user->ID;
    $super_user = get_option('framework_vergo_super_user');
    
    $themename =  get_option('vergo_themename');      
    $shortname =  get_option('vergo_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'vergothemes' ) {
		if (isset($_REQUEST['vergo_save']) && 'reset' == $_REQUEST['vergo_save']) {

			$options =  get_option('vergo_template'); 
			vergo_reset_options($options,'vergothemes');
			header("Location: admin.php?page=vergothemes&reset=true");
			die;
		}
    }
    elseif ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'vergothemes_framework_settings' ) {
		if (isset($_REQUEST['vergo_save']) && 'reset' == $_REQUEST['vergo_save']) {
		
			$options = get_option('vergo_framework_template'); 
			vergo_reset_options($options);
			header("Location: admin.php?page=vergothemes_framework_settings&reset=true");
			die;
		}
    }
	...
	if(get_option('framework_vergo_backend_icon')) { $icon = get_option('framework_vergo_backend_icon'); }
	else { $icon = get_template_directory_uri(). '/functions/images/vergo-icon.png'; }
	
    if(function_exists('add_object_page'))
    {
     $vergopage = add_object_page ('Page Title', $themename, 'edit_pages','vergothemes', 'vergothemes_options_page', $icon);
    }
    else
    {
    }
	...
	add_action("admin_print_scripts-$vergopage", 'vergo_load_only');

     
} 

add_action('admin_menu', 'vergothemes_add_admin');

/*-----------------------------------------------------------------------------------*/
/* Vergo Reset Function - vergo_reset_options */
/*-----------------------------------------------------------------------------------*/

function vergo_reset_options($options,$page = ''){
	...
	//When Theme Options page is reset - Add the vergo_options option
	if($page == 'vergothemes'){
		$query_inner .= " OR option_name = 'vergo_options'";
	}
...
/*-----------------------------------------------------------------------------------*/
/* Framework options panel - vergothemes_options_page */
/*-----------------------------------------------------------------------------------*/

function vergothemes_options_page(){
	...
	$update_message = '';
    if(get_option('framework_vergo_theme_version_checker') == 'true') {
        $update_message = vergothemes_version_checker($local_version);
    }

?>
<div class="wrap" id="vergo_container">
<div id="vergo-popup-save" class="vergo-save-popup"><div class="vergo-save-save">Options Updated</div></div>
<div id="vergo-popup-reset" class="vergo-save-popup"><div class="vergo-save-reset">Options Reset</div></div>
    <form action="" enctype="multipart/form-data" id="vergoform">
        <div id="header">
        ...
        $return = vergothemes_machine($options);
        ?>
        ...
        <form action="<?php echo esc_html( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="vergoform-reset">
            ...
            <input type="hidden" name="vergo_save" value="reset" /> 
            </span>
        </form>
...
function vergo_load_only() {

	add_action('admin_head', 'vergo_admin_head');
	...
	function vergo_admin_head() { 
		...
		jQuery(document).ready(function(){
			...
			//JQUERY DATEPICKER
			jQuery('.vergo-input-calendar').each(function (){
			...
			jQuery('.vergo-input-time').each(function (){
				jQuery('#' + jQuery(this).attr('id')).mask("99-9999999");
			});
			
			//Color Picker
			<?php $options = get_option('vergo_template');
		...
		<script type="text/javascript">
			...
			jQuery('#expand_options').click(function(){
				if(flip == 0){
					flip = 1;
					jQuery('#vergo_container #vergo-nav').hide();
					jQuery('#vergo_container #content').width(755);
					jQuery('#vergo_container .group').add('#vergo_container .group h2').show();
	
					jQuery(this).text('[-]');
					
				} else {
					flip = 0;
					jQuery('#vergo_container #vergo-nav').show();
					jQuery('#vergo_container #content').width(595);
					jQuery('#vergo_container .group').add('#vergo_container .group h2').hide();
					jQuery('#vergo_container .group:first').show();
					jQuery('#vergo_container #vergo-nav li').removeClass('current');
					jQuery('#vergo_container #vergo-nav li:first').addClass('current');
				...
				jQuery('.vergo-radio-img-img').click(function(){
					jQuery(this).parent().parent().find('.vergo-radio-img-img').removeClass('vergo-radio-img-selected');
					jQuery(this).addClass('vergo-radio-img-selected');
					
				});
				jQuery('.vergo-radio-img-label').hide();
				jQuery('.vergo-radio-img-img').show();
				jQuery('.vergo-radio-img-radio').hide();
				jQuery('#vergo-nav li:first').addClass('current');
				jQuery('#vergo-nav li a').click(function(evt){
				
						jQuery('#vergo-nav li').removeClass('current');
				...
				if('<?php if(isset($_REQUEST['reset'])) { echo $_REQUEST['reset'];} else { echo 'false';} ?>' == 'true'){
					...
					var reset_popup = jQuery('#vergo-popup-reset');
					reset_popup.fadeIn();
			...
			jQuery('#vergo-popup-save').center();
			jQuery('#vergo-popup-reset').center();
			...
			new AjaxUpload(clickedID, {
				  ...
				  onComplete: function(file, response) {
					...
					if(response.search('Upload Error') > -1){
						...
						var buildReturn = '<img class="hide vergo-option-image" id="image_'+clickedID+'" src="'+response+'" alt="" />';
			...
			jQuery('#vergoform').submit(function(){
				
					function newValues() {
					  var serializedValues = jQuery("#vergoform").serialize();
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'vergothemes'){ ?>
						type: 'options',
						<?php } ?>
					...
					jQuery.post(ajax_url, data, function(response) {
						var success = jQuery('#vergo-popup-save');
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - vergo_ajax_callback */
/*-----------------------------------------------------------------------------------*/

add_action('wp_ajax_vergo_ajax_post_action', 'vergo_ajax_callback');

function vergo_ajax_callback() {
	...
	elseif ($save_type == 'options' OR $save_type == 'seo' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
				...
				if($id == 'framework_vergo_import_options'){
	...
	if( $save_type == 'options' OR $save_type == 'framework' ){
		...
		foreach ($results as $result){
				...
				if(is_serialized($value)) {
					...
					$value = $temp_options;
					$vergo_array[$name] = $vergo_array_option;
				} else {
					$vergo_array[$name] = $value;
				}
		...
		update_option('vergo_settings_encode',$output);
...
/*-----------------------------------------------------------------------------------*/
/* Generates The Options - vergothemes_machine */
/*-----------------------------------------------------------------------------------*/

function vergothemes_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			...
			if ( $std != "") { $val = $std; }
			$output .= '<input class="vergo-input" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. $val .'" />';
		break;
		
		case 'select':

			$output .= '<select class="vergo-input" name="'. $value['id'] .'" id="'. $value['id'] .'">';
		...
		case 'calendar':
			...
			$std = get_option($value['id']);
			if ( $std != "") { $val = $std; }
            $output .= '<input class="vergo-input-calendar" type="text" name="'.$value['id'].'" id="'.$value['id'].'" value="'.$val.'">';
		...
		case 'time':
			...
			$std = get_option($value['id']);
			if ( $std != "") { $val = $std; }
			$output .= '<input class="vergo-input-time" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
		break;
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				$output .= '<textarea class="vergo-input" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';
		...
		case "radio":
			 ...
			 { 
				...
				$output .= '<input class="vergo-input vergo-radio" type="radio" name="'. $value['id'] .'" value="'. $key .'" '. $checked .' />' . $option .'<br />';
		...
		case "checkbox": 
		   ...
		   $checked = '';
			...
			$output .= '<input type="checkbox" class="checkbox vergo-input" name="'.  $value['id'] .'" id="'. $value['id'] .'" value="true" '. $checked .' />';
		...
		case "multicheck":
			...
			$vergo_key = $value['id'] . '_' . $key;
			$saved_std = get_option($vergo_key);
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox vergo-input" name="'. $vergo_key .'" id="'. $vergo_key .'" value="true" '. $checked .' /><label for="'. $vergo_key .'">'. $option .'</label><br />';
										
			}
		break;
		case "upload":
			
			$output .= vergothemes_uploader_function($value['id'],$value['std'],null);
			
		break;
		case "upload_min":
			
			$output .= vergothemes_uploader_function($value['id'],$value['std'],'min');
		...
		case "color":
			...
			$output .= '<input class="vergo-color" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
		break;   
		...
		case "typography":
			...
			$output .= '<select class="vergo-typography vergo-typography-size" name="'. $value['id'].'_size" id="'. $value['id'].'_size">';
				for ($i = 9; $i < 71; $i++){ 
			...
			$output .= '<select class="vergo-typography vergo-typography-unit" name="'. $value['id'].'_unit" id="'. $value['id'].'_unit">';
			...
			*/
			...
			$output .= '<select class="vergo-typography vergo-typography-face" name="'. $value['id'].'_face" id="'. $value['id'].'_face">';
			...
			$output .= '<option value="&quot;Roboto&quot;, sans-serif" '. $font53 .'>Roboto***</option>';
			$output .= '<option value="&quot;Source Sans Pro&quot;, sans-serif" '. $font54 .'>Source Sans Pro***</option>';
			$output .= '<option value="&quot;Arial Black&quot;, sans-serif" '. $font55 .'>Arial Black*</option>';
			$output .= '<option value="&quot;Abril Fatface&quot;, sans-serif" '. $font56 .'>Abril Fatface***</option>';
			$output .= '</select>';
			...
			$output .= '<select class="vergo-typography vergo-typography-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
			...
			$output .= '<input class="vergo-color vergo-typography vergo-typography-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
		...
		case "border":
			...
			$output .= '<select class="vergo-border vergo-border-width" name="'. $value['id'].'_width" id="'. $value['id'].'_width">';
				for ($i = 0; $i < 21; $i++){ 
					if($val == $i){ $active = 'selected="selected"'; } else { $active = ''; }
					$output .= '<option value="'. $i .'" ' . $active . '>'. $i .'px</option>'; }
			$output .= '</select>';
			...
			$output .= '<select class="vergo-border vergo-border-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
			...
			$output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector"><div></div></div>';
			$output .= '<input class="vergo-color vergo-border vergo-border-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
		...
		case "images":
			...
			foreach ($value['options'] as $key => $option) 
			 ...
			 $i++;
				 ...
				 $selected = '';
				   if($select_value != '') {
						if ( $select_value == $key) { $checked = ' checked'; $selected = 'vergo-radio-img-selected'; } 
				    } else {
						if ($value['std'] == $key) { $checked = ' checked'; $selected = 'vergo-radio-img-selected'; }
						elseif ($i == 1  && !isset($select_value)) { $checked = ' checked'; $selected = 'vergo-radio-img-selected'; }
						elseif ($i == 1  && $value['std'] == '') { $checked = ' checked'; $selected = 'vergo-radio-img-selected'; }
						else { $checked = ''; }
					}	
				
				$output .= '<span>';
				$output .= '<input type="radio" id="vergo-radio-img-' . $value['id'] . $i . '" class="checkbox vergo-radio-img-radio" value="'.$key.'" name="'. $value['id'].'" '.$checked.' />';
				$output .= '<div class="vergo-radio-img-label">'. $key .'</div>';
				$output .= '<img src="'.$option.'" alt="" class="vergo-radio-img-img '. $selected .'" onClick="document.getElementById(\'vergo-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
				$output .= '</span>';
		...
		case "heading":
			...
			$jquery_click_hook = "vergo-option-" . $jquery_click_hook;
//			$jquery_click_hook = "vergo-option-" . str_replace("&","",str_replace("/","",str_replace(".","",str_replace(")","",str_replace("(","",str_replace(" ","",strtolower($value['name'])))))));
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					if($array['type'] == 'text') { // Only text at this point
						 ...
						 $output .= '<input class="input-text-small vergo-input" name="'. $id .'" id="'. $id .'" type="text" value="'. $std .'" />';  
...
/*-----------------------------------------------------------------------------------*/
/* Vergo Uploader - vergothemes_uploader_function */
/*-----------------------------------------------------------------------------------*/

function vergothemes_uploader_function($id,$std,$mod){
	...
	if($mod != 'min') { 
            ...
            $uploader .= '<input class="vergo-input" name="'. $id .'" id="'. $id .'_upload" type="text" value="'. $val .'" />';
	}
	...
	if(!empty($upload)){
    	...
    	$uploader .= '<a class="vergo-uploaded-image" href="'. $upload . '">';
    	$uploader .= '<img class="vergo-option-image" id="image_'.$id.'" src="'.$upload.'" alt="" />';
    	$uploader .= '</a>';
		}
	$uploader .= '<div class="clear"></div>' . "\n"; 
