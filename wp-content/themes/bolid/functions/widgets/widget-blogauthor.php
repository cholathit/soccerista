...
class BlogAuthorInfo extends WP_Widget {
   ...
   function BlogAuthorInfo() {
	   ...
	   parent::WP_Widget(false, __('Vergo - Blog Author Info', 'vergo'),$widget_ops);      
   }
   ...
   function widget($args, $instance) {  
	...
	?>
		...
		<p style="margin:0 0 30px 0;"><?php echo $bio; ?><br/>
		<?php if ( $read_more_url ) echo '<a class="comment-reply-link" style="float:right; href="' . $read_more_url . '">' . $read_more_text . '</a>'; ?></p>
   ...
   function form($instance) {   
		...
		<p>
		   <label for="<?php echo $this->get_field_id('bio'); ?>"><?php _e('Bio:','vergo'); ?></label>
		...
		<p>
		   <label for="<?php echo $this->get_field_id('custom_email'); ?>"><?php _e('<a href="http://www.gravatar.com/">Gravatar</a> E-mail:','vergo'); ?></label>
		...
		<p>
		   <label for="<?php echo $this->get_field_id('avatar_size'); ?>"><?php _e('Gravatar Size:','vergo'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('avatar_align'); ?>"><?php _e('Gravatar Alignment:','vergo'); ?></label>
			<select name="<?php echo $this->get_field_name('avatar_align'); ?>" class="widefat" id="<?php echo $this->get_field_id('avatar_align'); ?>">
				<option value="left" <?php if($avatar_align == "left"){ echo "selected='selected'";} ?>><?php _e('Left', 'vergo'); ?></option>
				<option value="right" <?php if($avatar_align == "right"){ echo "selected='selected'";} ?>><?php _e('Right', 'vergo'); ?></option>            
			</select>
		</p>
		<p>
		   <label for="<?php echo $this->get_field_id('read_more_text'); ?>"><?php _e('Read More Text (optional):','vergo'); ?></label>
		...
		<p>
		   <label for="<?php echo $this->get_field_id('read_more_url'); ?>"><?php _e('Read More URL (optional):','vergo'); ?></label>
